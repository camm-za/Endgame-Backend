...
class functions {
    ...
    getContentPages(req) {
        ...
        const contentpages = JSON.parse(fs.readFileSync(path.join(__dirname, "../", "local", "contentpages.json")).toString());
        let Language = "en";
        try {
    ...
    getItemShop() {
        const catalog = JSON.parse(fs.readFileSync(path.join(__dirname, "../../", "local", "Storefront", "catalog.json"), "utf8"));
        const CatalogConfig = JSON.parse(fs.readFileSync(path.join(__dirname, "../../", "local", "Storefront", "catalog.json"), "utf8"));
        try {
