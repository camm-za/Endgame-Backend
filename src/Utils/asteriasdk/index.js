...
class Asteria {
  ...
  constructor(options) {
    ...
    if (!this.collectAnonStats) {
      throw new TypeError(
        "The 'collectAnonStats' option must be a boolean value of either true or false."
      );
    }
    this.uid = this.collectAnonStats ? generateUniqueIdentifier() : "disabled";
    this.usedURL = new URL(
      options.usedURL ? options.usedURL : "https://fortnite.rest/"
    );
  ...
  async getEntity(key, value, ignoreErrors) {
    ...
    if (!req.ok) {
      if (req.status === 404) {
        ...
        if (this.throwErrors)
          throw new Error(
            `Entity $could not be found by Key ${key} and Value ${value}. Please check your Key and Value.`
          );
      } else {
        if (ignoreErrors) return;
        throw new Error(
          `Error ${req.status} occurred while fetching cosmetic with key ${key} and value ${value}.`
        );
      }
    }
