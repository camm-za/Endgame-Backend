...
import { verifyToken, DateAddHours } from "../User/tokenManager/tokenVerify.js";
...
app.get("/affiliate/api/public/affiliates/slug/:slug", async (req, res) => {
  ...
  if (
    (index = codes.findIndex(
      (f) => f.code.toLowerCase() == req.params.slug.toLowerCase()
    )) == -1
  )
    return error.createError(
      "errors.com.epicgames.sac.invalid",
      `Invalid code.`,
      [],
      1000,
      undefined,
  ...
  var user = User.findOne({ accountId: codes[index].accountId });
  if (!user)
    return error.createError(
      "errors.com.epicgames.sac.invalid",
      `Invalid code.`,
...
app.get('/fortnite/api/game/v2/profile/*', (req, res, next) => {
            ...
            if (
              ...
              for (var i = 0; i < EndingTier; i++) {
                ...
                for (var item in FreeTier) {
                  ...
                  if (typeof item === 'string' && item.toLowerCase().startsWith("homebasebanner")) {
                    for (var key in profile.items) {
                        if (
                            profile.items[key] &&
                            profile.items[key].templateId &&
                            typeof profile.items[key].templateId === 'string' &&
                            profile.items[key].templateId.toLowerCase() === item.toLowerCase()
                        ) {
