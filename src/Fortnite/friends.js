...
import { friends } from "../Utils/structs/friend.js";
...
app.post(
  ...
  async (req, res) => {
    ...
    if (sender.list.incoming.find((i) => i.accountId == receiver?.accountId)) {
      if (
        !(await friends.acceptFriendReq(sender.accountId, receiver.accountId))
      )
...
app.all(
  ...
  async (req, res) => {
    ...
    switch (req.method) {
      case "PUT":
        if (
          !aliasPattern.test(req.rawBody) ||
          req.rawBody.length < 3 ||
          req.rawBody.length > 16
        )
          return error.createError(
