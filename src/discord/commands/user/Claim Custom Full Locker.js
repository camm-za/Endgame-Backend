...
const REQUIRED_ROLE_ID = '1363931815609630771';
...
async function sendWebhookLog(discordUser, action, status, details = {}) {
  const webhookEmbed = {
    embeds: [
      {
        title: "Custom Full Locker Claim",
        description: `A user attempted to claim a custom full locker: ${status}`,
...
export const data = new SlashCommandBuilder()
  .setName("claim-custom-full-locker")
  .setDescription(
    "Claim Custom Full Locker"
  )
...
export async function execute(interaction) {
  ...
  const allItems = destr(
    fs.readFileSync(
      path.join(__dirname, "../../../../Config/FL/customathena.json"),
      "utf8"
    )
  );
  if (!allItems) {
  ...
  try {
    ...
    commonCoreProfile.items[giftBoxId] = {
      ...
      attributes: {
        ...
        params: {
          ...
          userMessage: 'Enjoy Custom Full Locker!'
        },
    ...
    await sendWebhookLog(interaction.user, 'Claim Custom Full Locker', 'Success', { ItemsClaimed: Object.keys(allItems.items).length.toString() });
    await interaction.editReply({
      content: `You Have Successfully Claimed Custom Full Locker!`,
      ephemeral: true,
    });
  } catch (error) {
    await sendWebhookLog(interaction.user, 'Claim Attempt', 'Failed', { Reason: error.message });
    log.error(`Error claiming custom full locker for user ${userId}: ${error.message}`);
    await interaction.editReply({
      content: 'An error occurred while claiming the custom full locker.',
      ephemeral: true,
    });
  }
}